...
class TodoServiceImplTest {
    ...
    public void whenCreateTodoAuthenticated_ThenReturnTrue() {
        final Principal mockUser = mock(UserPrincipal.class);
        when(mockUser.getName()).thenReturn("username");
        final AuthenticationService mockAuthService = mock(AuthenticationService.class);
        when(mockAuthService.getCurrentUser()).thenReturn(Optional.of(mockUser));
        final UserRepository mockUserRepository = mock(UserRepository.class);
        when(mockUserRepository.findByUsername(mockUser.getName())).thenReturn(Optional.of(mock(User.class)));
        ...
        final boolean result = assertDoesNotThrow(() -> todoService.createTodo(new TodoData("text todo")));
        assertTrue(result);
  }


    @Test
    public void whenCreateTodoNotAuthenticated_ThenThrowsAuthenticationException() {
        ...
        assertThrows(AuthenticationException.class, () -> todoService.createTodo(new TodoData("text todo")));
    }

    @Test
    public void whenCreateTodoUnknownAuthentication_ThenAuthenticationException() {
        ...
        when(mockAuthenticationService.getCurrentUser()).thenReturn(Optional.of(principal));
